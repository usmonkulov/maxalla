...
class UserController extends AdminController
{
    ...
    public function actionCreate()
    {
        ...
        if($file = UploadedFile::getInstance($user,'avatar')){
            $user->avatar = $user->upload(strtolower(Yii::$app->controller->id).'ss',$file);
        }
        if (!$user->save()) {
            return $this->render('create', ['user' => $user]);
        }
    ...
    public function actionUpdate($id)
    {
        ...
        if($file = UploadedFile::getInstance($user,'avatar')){
                $user->avatar = $user->upload(strtolower(Yii::$app->controller->id).'ss',$file);
                if(file_exists($user->getOldAttribute('avatar')))
                    unlink($user->getOldAttribute('avatar'));
            }else{
                $user->avatar = $user->getOldAttribute('avatar');
        }      
